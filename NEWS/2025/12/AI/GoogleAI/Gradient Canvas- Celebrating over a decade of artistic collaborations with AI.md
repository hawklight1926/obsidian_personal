---
url: https://blog.google/technology/ai/google-gradient-canvas-ai-art/
date: 2025-12-11T21:30:00Z
updated: 
category: AI
rss: GoogleAI
image: https://storage.googleapis.com/gweb-uniblog-publish-prod/images/Gradient_Canvas_hero_FS1ek3X.max-600x600.format-webp.webp
summary: |-
  GoogleはAIとアーティストの10年以上にわたる協働を記念し、13点の新作を展示する「Gradient Canvas」を公開しました。
  ・展示作品はサンフランシスコ・ベイエリアの生態系に着想を得ており、AIが人間の知覚、機械の視覚、自然世界をつなぐ橋渡しとなる可能性を探っています。
  ・世界各国のアーティストがGoogleのツールやAIを活用し、人間と自然、機械の共進化する関係性を問いかけています。
  ・オンライン展示はGoogle Arts & Cultureで閲覧可能で、物理展示はカリフォルニア州マウンテンビューのGradient Canopyオフィスで開催されています。
read: false
ignored: false
pinned: false
memo: ""
---

![Gradient Canvas: Celebrating over a decade of artistic collaborations with AI](https://storage.googleapis.com/gweb-uniblog-publish-prod/images/Gradient_Canvas_hero_FS1ek3X.max-600x600.format-webp.webp)

## 要約
GoogleはAIとアーティストの10年以上にわたる協働を記念し、13点の新作を展示する「Gradient Canvas」を公開しました。
・展示作品はサンフランシスコ・ベイエリアの生態系に着想を得ており、AIが人間の知覚、機械の視覚、自然世界をつなぐ橋渡しとなる可能性を探っています。
・世界各国のアーティストがGoogleのツールやAIを活用し、人間と自然、機械の共進化する関係性を問いかけています。
・オンライン展示はGoogle Arts & Cultureで閲覧可能で、物理展示はカリフォルニア州マウンテンビューのGradient Canopyオフィスで開催されています。
