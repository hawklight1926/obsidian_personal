...
summary: |-
  Gemini 3 Proは、文書、空間、画面、動画理解において最先端の性能を発揮する、Googleの最も高度なマルチモーダルAIモデルです。
  ・文書理解では、複雑なレイアウトや手書き文字の認識から、表や図表を跨いだ多段階の推論までを高い精度で実行できます。
  ・空間・画面理解では、物理的世界やOS画面の構造を把握し、タスク自動化などの応用が可能です。
  ・動画理解では、高フレームレート処理と「思考」モードにより、複雑な因果関係の推論を含む高度な解析を実現しています。
read: false
ignored: false
pinned: false
memo: ""
