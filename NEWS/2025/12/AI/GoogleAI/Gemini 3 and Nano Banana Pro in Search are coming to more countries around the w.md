---
url: https://blog.google/products/search/gemini-3-ai-mode-more-countries/
date: 2025-12-01T20:00:00Z
updated: 
category: AI
rss: GoogleAI
image: https://storage.googleapis.com/gweb-uniblog-publish-prod/images/Gemini3inGoogleSearch_Social.max-600x600.format-webp.webp
summary: |-
  Googleが最高性能のAIモデル「Gemini 3」を、約120の国と地域のGoogle検索AIモードに英語で提供開始しました。
  - Google AI Pro/Ultra加入者は、Gemini 3 Proの高度な推論能力により、複雑な質問のニュアンスを理解できます。
  - マルチモーダル理解とエージェント的コーディング機能により、動的な視覚的レイアウトやインタラクティブなツールが生成されます。
  - 画像生成モデル「Nano Banana Pro」も拡大導入され、インフォグラフィックなどの可視化が可能です。
read: false
ignored: false
pinned: false
memo: ""
---

![Gemini 3 and Nano Banana Pro in Search are coming to more countries around the world.](https://storage.googleapis.com/gweb-uniblog-publish-prod/images/Gemini3inGoogleSearch_Social.max-600x600.format-webp.webp)

## 要約
Googleが最高性能のAIモデル「Gemini 3」を、約120の国と地域のGoogle検索AIモードに英語で提供開始しました。
- Google AI Pro/Ultra加入者は、Gemini 3 Proの高度な推論能力により、複雑な質問のニュアンスを理解できます。
- マルチモーダル理解とエージェント的コーディング機能により、動的な視覚的レイアウトやインタラクティブなツールが生成されます。
- 画像生成モデル「Nano Banana Pro」も拡大導入され、インフォグラフィックなどの可視化が可能です。
