---
url: https://japan.zdnet.com/article/35241496/
date: 2025-12-11T07:07:00+09:00
updated: 
category: Tech
rss: ZDNetJapan
image: https://japan.zdnet.com/storage/2025/12/11/8f4d55b99954ec9e7229cb3a3e0d9348/t/184/138/d/gettyimages-1651092661_hero.jpg
summary: |-
  AIチャットボット市場では、依然としてChatGPTが首位を維持する一方、Googleの「Gemini」とxAIの「Grok」が驚異的な成長率で勢力を伸ばしています。
  - Geminiはデスクトップ訪問者数が前年比971％増、Grokもモバイルで472％増と大幅成長しました。
  - 両者の成長要因としては、Gemini 3モデルのリリースや「Think Mode」の導入など、モデルの大幅なアップグレードが寄与しています。
  - 一方、Microsoftの「Copilot」はブラウザー経由の訪問者数が減少し、AIオーディオツール（ElevenLabs、Sunoなど）の成長が著しい分野として注目されています。
read: false
ignored: false
pinned: false
memo: ""
---

![AIチャットボット市場の勢力図に変化--「Gemini」と「Grok」が成長をけん引](https://japan.zdnet.com/storage/2025/12/11/8f4d55b99954ec9e7229cb3a3e0d9348/t/184/138/d/gettyimages-1651092661_hero.jpg)

## 要約
AIチャットボット市場では、依然としてChatGPTが首位を維持する一方、Googleの「Gemini」とxAIの「Grok」が驚異的な成長率で勢力を伸ばしています。
- Geminiはデスクトップ訪問者数が前年比971％増、Grokもモバイルで472％増と大幅成長しました。
- 両者の成長要因としては、Gemini 3モデルのリリースや「Think Mode」の導入など、モデルの大幅なアップグレードが寄与しています。
- 一方、Microsoftの「Copilot」はブラウザー経由の訪問者数が減少し、AIオーディオツール（ElevenLabs、Sunoなど）の成長が著しい分野として注目されています。
