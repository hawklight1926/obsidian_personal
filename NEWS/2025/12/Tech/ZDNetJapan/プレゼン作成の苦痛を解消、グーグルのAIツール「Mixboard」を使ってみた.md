...
summary: |-
  Googleが提供するAIツール「Mixboard」は、写真やドキュメントからプレゼンテーションを自動生成するサービスです。
  ・ユーザーは素材をアップロードするか、プロンプトを入力することで、AIがプレゼンテーションを作成します。
  ・現在はパブリックベータ版として無料で提供されており、生成されたプレゼンはダウンロードや共有が可能です。
  ・動作にはChromiumベースのブラウザーが必要で、処理には一定の時間がかかります。
read: false
ignored: false
pinned: false
memo: ""
