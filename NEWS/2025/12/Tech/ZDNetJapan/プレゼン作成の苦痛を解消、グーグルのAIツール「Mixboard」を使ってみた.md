...
summary: |-
  GoogleのAIツール「Mixboard」は、写真やドキュメントからプレゼンテーションを自動生成するサービスです。
  - ユーザーは素材をアップロードするか、プロンプトを入力するだけで、AIが内容を解析してスライドを作成します。
  - 現在はパブリックベータ版として無料で提供されており、生成されたプレゼンはPDFなどでダウンロード可能です。
  - ただし、正常動作にはChromiumベースのブラウザが必要で、ファイル形式によってはサポートされない場合があります。
read: false
ignored: false
pinned: false
memo: ""
