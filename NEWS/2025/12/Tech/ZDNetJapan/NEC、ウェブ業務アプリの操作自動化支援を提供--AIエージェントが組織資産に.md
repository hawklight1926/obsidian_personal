---
url: https://japan.zdnet.com/article/35241236/
date: 2025-12-04T15:15:00+09:00
updated: 
category: Tech
rss: ZDNetJapan
image: https://japan.zdnet.com/storage/2025/12/04/2d9c261099b8aaf3b939fd1277f4dee7/t/184/138/d/20251204_adobestock_261206839_1280x960.jpg
summary: |-
  NECはウェブ業務アプリの操作ノウハウを自動抽出・共有するAIエージェント「cotomi Act」を2026年1月から提供します。
  - 従来の自動化技術では対応が難しかった企業固有の業務ルールや暗黙知を学習し、操作を自動化します。
  - 社員の日常業務からノウハウを自動的に抽出し、教え込みやマニュアル作成が不要な組織資産として蓄積・活用できます。
  - 国際ベンチマークで人間を上回る性能を実現し、経費精算など複雑な判断を要する業務の自動化も可能としています。
read: false
ignored: false
pinned: false
memo: ""
---

![NEC、ウェブ業務アプリの操作自動化支援を提供--AIエージェントが組織資産に](https://japan.zdnet.com/storage/2025/12/04/2d9c261099b8aaf3b939fd1277f4dee7/t/184/138/d/20251204_adobestock_261206839_1280x960.jpg)

## 要約
NECはウェブ業務アプリの操作ノウハウを自動抽出・共有するAIエージェント「cotomi Act」を2026年1月から提供します。
- 従来の自動化技術では対応が難しかった企業固有の業務ルールや暗黙知を学習し、操作を自動化します。
- 社員の日常業務からノウハウを自動的に抽出し、教え込みやマニュアル作成が不要な組織資産として蓄積・活用できます。
- 国際ベンチマークで人間を上回る性能を実現し、経費精算など複雑な判断を要する業務の自動化も可能としています。
