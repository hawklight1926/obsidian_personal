---
url: https://japan.zdnet.com/article/35241299/
date: 2025-12-05T17:03:00+09:00
updated: 
category: Tech
rss: ZDNetJapan
image: https://japan.zdnet.com/storage/2025/04/23/27bbac9b83c7ded844ea39e720c2d340/t/184/138/d/thumb_sp_22carbon-neutral_1280.jpg
summary: |-
  NTTデータとCBAは、廃棄物処理に伴う温室効果ガス排出量（スコープ3カテゴリー5）の効率的な算定で連携します。
  - NTTデータの排出量可視化プラットフォーム「C-Turtle」とCBAの廃棄物管理プラットフォーム「CBA wellfest」を連携させます。
  - 京王電鉄を対象とした実証では、データ連携により集合作業負荷の軽減効果を確認しました。
  - 両社は本格的な機能開発を進め、2026年4月に新サービスとして提供する予定です。
read: false
ignored: false
pinned: false
memo: ""
---

![NTTデータとCBA、廃棄物処理での温室効果ガス排出量算定で連携](https://japan.zdnet.com/storage/2025/04/23/27bbac9b83c7ded844ea39e720c2d340/t/184/138/d/thumb_sp_22carbon-neutral_1280.jpg)

## 要約
NTTデータとCBAは、廃棄物処理に伴う温室効果ガス排出量（スコープ3カテゴリー5）の効率的な算定で連携します。
- NTTデータの排出量可視化プラットフォーム「C-Turtle」とCBAの廃棄物管理プラットフォーム「CBA wellfest」を連携させます。
- 京王電鉄を対象とした実証では、データ連携により集合作業負荷の軽減効果を確認しました。
- 両社は本格的な機能開発を進め、2026年4月に新サービスとして提供する予定です。
