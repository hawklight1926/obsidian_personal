---
url: https://japan.zdnet.com/article/35241116/
date: 2025-12-02T09:33:00+09:00
updated:
category: Tech
rss: ZDNetJapan
image: https://japan.zdnet.com/storage/2025/12/02/96eaec3eae414a58719e49d6ada6ec54/t/184/138/d/chatgpt-ads_1280.jpg
summary: |-
<<<<<<< HEAD
  OpenAIがChatGPTへの広告導入を検討している可能性が示唆されています。  
  ・Androidアプリのベータ版コードに広告関連の記述が複数確認されました。  
  ・CEOのSam Altman氏は以前は消極的でしたが、最近では条件付きで容認する発言をしています。  
  ・広告導入の背景には、同社の大幅な損失という収益面の課題が指摘されています。
read: true
||||||| 758c5fd
  OpenAIがChatGPTへの広告導入を検討している可能性が示唆されています。  
  ・Androidアプリのベータ版コードに広告関連の記述が複数確認されました。  
  ・CEOのSam Altman氏は以前は消極的でしたが、最近では条件付きで容認する発言をしています。  
  ・広告導入の背景には、同社の大幅な損失という収益面の課題が指摘されています。
read: false
=======
  OpenAIがChatGPTへの広告導入を検討している可能性があります。
  ・Androidアプリのベータ版コードに広告関連の記述が複数確認されました。
  ・CEOのサム・アルトマン氏は以前は消極的でしたが、最近の見解では条件付きで容認する姿勢を示しています。
  ・広告導入の背景には、同社が大きな営業損失を計上しているという収益面の課題が指摘されています。
read: false
>>>>>>> origin/main
ignored: false
pinned: false
memo: ""
---

![OpenAI、「ChatGPT」に広告を導入か--コード内に広告を示唆する記述](https://japan.zdnet.com/storage/2025/12/02/96eaec3eae414a58719e49d6ada6ec54/t/184/138/d/chatgpt-ads_1280.jpg)

## 要約
OpenAIがChatGPTへの広告導入を検討している可能性があります。
・Androidアプリのベータ版コードに広告関連の記述が複数確認されました。
・CEOのサム・アルトマン氏は以前は消極的でしたが、最近の見解では条件付きで容認する姿勢を示しています。
・広告導入の背景には、同社が大きな営業損失を計上しているという収益面の課題が指摘されています。
