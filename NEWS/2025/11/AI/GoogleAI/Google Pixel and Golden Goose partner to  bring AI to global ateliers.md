---
url: https://blog.google/products/pixel/google-pixel-and-golden-goose-partner-to-bring-ai-to-global-ateliers/
date: 2025-11-13T08:00:00Z
updated: 
category: AI
rss: GoogleAI
image: https://storage.googleapis.com/gweb-uniblog-publish-prod/images/Screenshot_2025-11-13_10.23.26_.max-600x600.format-webp.webp
summary: |-
  Google PixelとGolden Gooseが提携し、AIを活用した靴のパーソナライズ体験を世界40店舗以上で提供します。
  - 店内ではGoogle PixelとGeminiアプリを使用し、自分の名前や好みのスタイルを取り入れたデジタルアートワークを生成できます。
  - 生成されたデザインはGolden Gooseの職人によってスニーカーに手作業で施され、伝統的技法とデジタル革新が融合されます。
  - 両社は技術と創造性の連携により、ファッション産業における新たな自己表現の可能性を追求しています。
read: false
ignored: false
pinned: false
memo: ""
---

![Google Pixel and Golden Goose partner to  bring AI to global ateliers](https://storage.googleapis.com/gweb-uniblog-publish-prod/images/Screenshot_2025-11-13_10.23.26_.max-600x600.format-webp.webp)

## 要約
Google PixelとGolden Gooseが提携し、AIを活用した靴のパーソナライズ体験を世界40店舗以上で提供します。
- 店内ではGoogle PixelとGeminiアプリを使用し、自分の名前や好みのスタイルを取り入れたデジタルアートワークを生成できます。
- 生成されたデザインはGolden Gooseの職人によってスニーカーに手作業で施され、伝統的技法とデジタル革新が融合されます。
- 両社は技術と創造性の連携により、ファッション産業における新たな自己表現の可能性を追求しています。
