...
summary: |-
  Googleがクラウド上のGeminiモデルの能力を活用しつつ、ユーザーデータを保護する新たなAI処理プラットフォーム「Private AI Compute」を発表しました。
  - 高度なAI体験にはクラウド処理が不可欠だが、個人データはユーザー専用に隔離され、Googleを含む他者からもアクセス不能となります。
  - これは、オンデバイス処理と同等のプライバシー保証をクラウドで実現するもので、複数のセキュリティ層で保護されています。
  - 具体的には、Pixel端末のMagic CueやRecorderアプリなどが、より高度でプライベートな機能を提供するために既に活用されています。
read: false
ignored: false
pinned: false
memo: ""
