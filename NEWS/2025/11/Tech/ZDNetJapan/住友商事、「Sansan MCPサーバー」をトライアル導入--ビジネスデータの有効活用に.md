---
url: https://japan.zdnet.com/article/35240945/
date: 2025-11-27T15:29:00+09:00
updated: 
category: Tech
rss: ZDNetJapan
image: https://japan.zdnet.com/storage/2025/02/13/a79d190ed078238bb849c8f1683cd326/t/184/138/d/250213_bigdata_as_1280_1088481721.jpeg
summary: |-
  住友商事は生成AI活用プロジェクトの一環として「Sansan MCPサーバー」をトライアル導入しました。
  - Sansanに蓄積された名刺や商談履歴などのビジネスデータをMicrosoft 365 Copilotで活用できるようにします。
  - MCP（Model Context Protocol）を利用し、外部データとAIのシームレスな連携を実現します。
  - 会議準備や提案資料作成など、営業活動における情報活用の高度化が期待されています。
read: false
ignored: false
pinned: false
memo: ""
---

![住友商事、「Sansan MCPサーバー」をトライアル導入--ビジネスデータの有効活用に](https://japan.zdnet.com/storage/2025/02/13/a79d190ed078238bb849c8f1683cd326/t/184/138/d/250213_bigdata_as_1280_1088481721.jpeg)

## 要約
住友商事は生成AI活用プロジェクトの一環として「Sansan MCPサーバー」をトライアル導入しました。
- Sansanに蓄積された名刺や商談履歴などのビジネスデータをMicrosoft 365 Copilotで活用できるようにします。
- MCP（Model Context Protocol）を利用し、外部データとAIのシームレスな連携を実現します。
- 会議準備や提案資料作成など、営業活動における情報活用の高度化が期待されています。
